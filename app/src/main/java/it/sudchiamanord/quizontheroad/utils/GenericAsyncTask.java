package it.sudchiamanord.quizontheroad.utils;

import android.os.AsyncTask;

/**
 * Defines a generic framework for running an AsyncTask that delegates
 * its operations to the @a Ops parameter.
 */
public class GenericAsyncTask<Params, Integer, Result, Ops extends
        GenericAsyncTaskOps<Params, Integer, Result>> extends AsyncTask<Params, Integer, Result>
{
    /**
     * Debugging tag used by the Android logger.
     */
    protected final String TAG = getClass().getSimpleName();
    
    /**
     * Params instance.
     */
    private Params[] mParam;

    /**
     * Reference to the enclosing Ops object.
     */
    protected Ops mOps;

    /**
     * Constructor initializes the field.
     */
    public GenericAsyncTask(Ops ops)
    {
	    mOps = ops;
    }

    /**
     * Runs on the UI thread before {@link #doInBackground}
     */
    protected void onPreExecute()
    {
        super.onPreExecute();

        publishProgress (0);
    }

    protected void publishProgress (int progress)
    {
        mOps.publishProgress (progress);
    }

    /**
     * Run in a background thread to avoid blocking the UI thread.
     */
    @SuppressWarnings("unchecked")
    protected Result doInBackground (Params... params)
    {
        mParam = params;
        return mOps.doInBackground (params);
    }

    /**
     * Process results in the UI Thread.
     */
    protected void onPostExecute (Result result)
    {
        mOps.onPostExecute (result, mParam);
    }
}
